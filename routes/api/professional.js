...
router.get('/', (req, res) => {
    db_pool.getInstance().query("SELECT * FROM resume_entries WHERE type = 'professional';", (error, results) => {
        if (error) {
            console.log(error)
        }
        return res.send({
...
router.put('/:id', validateToken, (req, res) => {
    ...
    db_pool.getInstance().query(`UPDATE resume_entries SET ${u.join(', ')} WHERE entry_id=${id} AND type='professional'`, (err, result) => {
        if (err) {
            console.log(err);
            res.status(500).send({error: true, message: "Internal server error"});
        } else {
...
router.delete('/:id', validateToken, (req, res) => {
    ...
    db_pool.getInstance().query(`DELETE FROM resume_entries WHERE entry_id=${id} AND type='professional'`, (err, result) => {
        if (err) {
            console.log(err);
            res.status(500).send({error: true, message: "Internal server error"});
        } else {
...
router.post('*', (req, res) => {
    res.status(404).send({error:true, message: 'Route not found in professional!'})
})
router.put('*', (req, res) => {
    res.status(404).send({error:true, message: 'Route not found in professional!'})
})
